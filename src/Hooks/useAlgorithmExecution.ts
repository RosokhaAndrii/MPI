...
export function useAlgorithmExecution({ algorithm, items, capacity }: UseAlgorithmExecutionProps) {
  ...
  const handlePlay = useCallback(() => {
    if (status === 'IDLE' || status === 'COMPLETED') {
      let generator;
      switch (algorithm) {
        case 'DP': generator = solveDPGenerator(items, capacity); break;
        case 'BRUTE_FORCE': generator = solveBruteForceGenerator(items, capacity); break;
        case 'GREEDY': generator = solveGreedyGenerator(items, capacity); break;
        case 'RECURSION': generator = solveRecursionGenerator(items, capacity); break;
        case 'BRANCH_AND_BOUND': generator = solveBranchGenerator(items, capacity); break;
        default: return;
      }
  ...
  return {
    ...
    finalResult,
    description: steps[currentStep]?.description || 'Натисніть Play для початку',
    speed,
    setSpeed,
    handlePlay,
    handlePause,
